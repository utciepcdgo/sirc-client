...
const filterBlocks = computed(() => {
	const result = getBlocks.value?.filter((block) => {
		...
		const hasCompensatories =
			block?.registrations?.stats?.compensatories > 0;

		if (
			showOnlyWithRegistrations.value &&
			showOnlyWithCompensatories.value
		) {
	...
	await axios
		.get(
			import.meta.env.VITE_SIRC_API_URI +
				`blocks/${block.id}?include=registrations`
		)
...
onMounted(async () => {
	...
	await store.fetchBlocks(); // Ensure blocks are loaded
	...
	<div class="p-6">
						...
						<DropdownMenuContent>
							<DropdownMenuItem>
								<div class="flex items-center space-x-2">
									<Switch
										id="only-with-compensatories"
										v-model:checked="
											showOnlyWithRegistrations
										" />
									<div class="flex flex-col">
										<Label for="only-with-compensatories"
											>Solo registrados</Label
										>
										<small
											>Muestre los municipios con un
											registro o más.</small
										>
									</div>
								</div>
							</DropdownMenuItem>
							<DropdownMenuItem>
								<div class="flex items-center space-x-2">
									<Switch
										id="only-with-registrations"
										v-model:checked="
											showOnlyWithCompensatories
										" />
									<div class="flex flex-col">
										<Label for="only-with-registrations"
											>Solo Medidas Compensatorias</Label
										>
										<small
											>Muestre los municipios con
											registros en alguna Medida
											Compensatoria.</small
										>
									</div>
								</div>
		...
		<AlertDialog v-model:open="store.blocksError">
			<AlertDialogContent class="min-w-[128]">
				...
				<p>
					Hubo un error al intentar cargar el contenido, contacte al
					administrador.
				</p>
				<p class="mt-3 text-sm p-0 leading-none">Detalles:</p>
				<small class="italic text-red-600 leading-none">{{
					store.blocksError
				}}</small>
			</AlertDialogContent>
		</AlertDialog>
			...
			<div v-for="block in filterBlocks" :key="block.id">
				<Card class="relative">
						...
						<div class="flex text-center justify-around">
							<div class="flex flex-col">
								...
								<span class="font-bold text-2xl mt-3">{{
									block.registrations.stats.women
								}}</span>
							</div>
							<div class="flex flex-col">
					...
					<CardFooter class="justify-between">
						<div
							class="flex space-x-2.5 md:space-x-1 sm:space-x-3.5 lg:space-x-1 xl:space-x-3.5 items-center">
							<Button variant="default" @click="openModal(block)">
								<IconPlus stroke-width="3" />
								Registrar
							</Button>
							<TooltipWrapper
								message="Vea los registros y gestione la información">
								<Button
									variant="secondary"
									@click="openModalDetails(block)">
									<IconInfoCircle />
									Detalles
								</Button>
							</TooltipWrapper>
						...
						<div class="flex items-center">
							<img
								:alt="block.entity.name"
								:src="block.entity.logo"
								class="rounded-full"
								width="24" />
						</div>
					</CardFooter>
		...
		<DialogRoot v-model:open="open" :update:open="open">
			<DialogScrollContent class="min-w-fit">
				...
				<DialogFooter class="flex items-center !justify-between">
					<div class="flex items-center space-x-2.5">
						<img
							:alt="selectedBlock.entity.name"
							:src="selectedBlock.entity.logo"
							class="rounded-md"
							width="32" />
						<div class="flex flex-col">
							<p class="text-sm">
		...
		<DialogRoot v-model:open="openDetails">
			<DialogScrollContent class="min-w-max">
				<DialogHeader>
					<DialogTitle
						>Registros en {{ blockdata.municipality.name }}
					</DialogTitle>
					<DialogDescription>
						Revise, edite o sustituya los registros del bloque
						seleccionado.
					</DialogDescription>
				</DialogHeader>
				<Tabs default-value="registrations">
					...
					<TabsContent value="information">
						...
						<h5
							class="text-md font-bold leading-none tracking-tight my-3">
							Propiedades
						</h5>
						<div class="flex flex-col mb-2">
							<span class="font-semibold"
								>Identificador del bloque:</span
							>
							<span>{{ blockdata.uuid }}</span>
							<span class="text-gray-600 leading-none text-xs"
								>Si tiene algún problema con la información
								presentada, comparta este identificador al área
								de soporte.</span
							>
						</div>
						<div class="flex flex-col">
							...
							<span class="text-gray-600"
								...
								{{
									blockdata.assignments.municipality
										? 'Sí'
										: 'No'
								}}</span
							>
							<span class="text-gray-600"
								>Sindicatura:
								{{
									blockdata.assignments.syndic ? 'Sí' : 'No'
								}}</span
							>
							<span class="text-gray-600"
								>Regidurías (Posiciones):
								{{ blockdata?.assignments?.councils }}</span
							>
						</div>
					</TabsContent>
				</Tabs>
