	...
	try {
		...
		if (
			...
			toast({
				title: 'Éxito',
				description:
					'No se permite cambiar la postulación o el carácter del registro tratándose de una sustitución.',
				variant: 'destructive',
			});
		...
		await axios.post(
			import.meta.env.VITE_SIRC_API_URI +
				`registrations/${props.registration.id}/substitute`,
			values
		);
							...
							<FormItem>
								...
								<FormControl>
									<Select
										...
										<SelectContent>
											<SelectGroup
												v-for="postulation in getPostulations"
												:key="postulation.id">
												<SelectItem
													:disabled="
														!postulation.active
									...
									<Select v-bind="componentField">
										...
										<SelectContent>
											<SelectGroup
												...
												<SelectItem
													:disabled="
														!registration?.block?.assignments.includes(
															i
														) &&
														registration
															?.assignments
