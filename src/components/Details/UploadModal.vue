...
const isFileUploaded = (file) => {
  ...
  return uploadedFiles.value.some(uploaded => uploaded.filetype_id === file.id);
};
    ...
    <DialogScrollContent class="min-w-[80%]">
      ...
      <Tabs default-value="upload-files">
          ...
          <div v-if="fileTypes.length">
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 items-end gap-4">
              <div v-for="file in fileTypes" :key="file.id">
                <FilePondUploader :disabled="isFileUploaded(file)" :document="file" :is-loaded="isFileUploaded(file)" :registration="registration"/>
              </div>
            </div>
          </div>
        ...
        <TabsContent value="uploaded-files">
          <DataTableUploadedFiles :files="uploadedFiles" :formats="fileTypes" />
        </TabsContent>
      </Tabs>
    </DialogScrollContent>
  </DialogRoot>
