...
const fetchFileTypes = async () => {
	...
	try {
		const response = await axios.get(
			import.meta.env.VITE_SIRC_API_URI + 'filetypes'
		);
...
const isFileUploaded = (file: Filetype) => {
	...
	return uploadedFiles.value.some(
		(uploaded) => uploaded.filetype.id === file.id
	);
};
...
function isAllowed(doc: Filetype) {
	...
	if (doc.allowed_to && typeof doc.allowed_to === 'object') {
		...
		if (doc?.allowed_to?.compensatory_id !== undefined) {
			matches =
				matches &&
				Number(props?.registration?.compensatory.id) ===
					Number(doc?.allowed_to?.compensatory_id);
		}

		if (doc?.allowed_to?.reelection !== undefined) {
			matches =
				matches &&
				Number(props?.registration?.reelection) ===
					Number(doc?.allowed_to?.reelection);
		}

		if (doc?.allowed_to?.sex !== undefined) {
			matches =
				matches &&
				Number(props?.registration?.sex.id) ===
					Number(doc?.allowed_to?.sex);
		}

		if (
			doc.allowed_to?.postulation_id !== undefined &&
			doc.allowed_to?.position_id !== undefined
		) {
			matches =
				matches &&
				Number(props?.registration?.postulation.id) ===
					Number(doc?.allowed_to?.postulation_id) &&
				Number(props?.registration?.position.id) ===
					Number(doc?.allowed_to?.position_id);
		}
		...
		<DialogScrollContent class="min-w-[80%]">
			<DialogHeader>
				...
				<small v-if="props.registration"
					...
					{{ props.registration.second_name }}</small
				>
			...
			<Tabs default-value="upload-files">
				...
				<TabsContent value="upload-files">
					<div v-if="fileTypes.length">
						<div
							class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 items-end gap-4">
				...
				<TabsContent value="uploaded-files">
					<DataTableUploadedFiles
						:files="uploadedFiles"
						:formats="fileTypes"
						@fileDeleted="fetchUploadedFiles" />
				</TabsContent>
			</Tabs>
		</DialogScrollContent>
	</DialogRoot>
