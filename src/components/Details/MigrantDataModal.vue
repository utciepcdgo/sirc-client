...
const onSubmit = handleSubmit(async (values) => {
	await axios
		.post(import.meta.env.VITE_SIRC_API_URI + 'migrants', values)
		.then(() => {
			toast({
	...
	await axios
		.get(
			import.meta.env.VITE_SIRC_API_URI +
				`registrations/${props.registration.id}`
		)
		.then((res) => {
	...
	<DialogRoot v-model:open="isOpen">
		<DialogScrollContent>
			<DialogHeader>
				...
				<DialogDescription>
					Indique el domicilio para {{ registration.name }}
					...
					<div>
						<FormField v-slot="{ componentField }" name="address">
							<FormItem>
								...
								<FormDescription
									>Indique la dirección completa en el
									extranjero</FormDescription
								>
								<FormMessage />
							</FormItem>
						</FormField>
					...
					<div>
						<FormField v-slot="{ componentField }" name="zip_code">
							<FormItem>
								...
								<FormDescription
									>Puede que el término sea diferente
									dependiendo del país, típicamente suele
									usarse como "Código Zip" (ZIP
							...
							<FormItem>
								...
								<FormControl>
									<Select
										v-bind="componentField"
										:default-value="
											initialRegistration?.migrant?.country?.id.toString()
										">
										<SelectTrigger>
											<SelectValue
								...
								<FormDescription
									>País seleccionado:
									{{
										initialRegistration?.migrant?.country
											?.name ?? '(Ninguno)'
									}}, puede modificar la
									selección.</FormDescription
								>
							</FormItem>
						</FormField>
					</div>
