...
const filePondServer = {
  ...
  process: (
    ...
    // Paso 1: Solicitar a la API la URL pre-firmada.
    ...
    const dataHeader = {
      ...
      postulation:
        props.registration.postulation.id === '5'
          ? props.registration.postulation.name.slice(0, 4).toUpperCase() + '_' + props.registration.council_number
          : props.registration.postulation.name.slice(0, 4).toUpperCase(),
      position: props.registration.position.name.slice(0, 4).toUpperCase(),
      candidacy:
        props.registration.name.replace(/\s+/g, '_').toUpperCase() +
        '_' +
        props.registration.first_name.replace(/\s+/g, '_').toUpperCase() +
        '_' +
        props.registration.second_name.replace(/\s+/g, '_').toUpperCase(),
    ...
    <div
      ...
      <div class="flex items-center space-x-3.5">
        ...
        <div class="flex flex-col flex-grow-0">
          ...
          <span class="[text-shadow:_0_1px_0_rgb(0_0_0_/_40%)] text-xs">
            Vaya a la pestaña "Documentación registrada" para gestionar la información.
          </span>
        </div>
      </div>
    </div>
