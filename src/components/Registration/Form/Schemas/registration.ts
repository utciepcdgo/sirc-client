...
export const registrationSchema = Yup.object({
  ...
  residence: Yup.object().shape({
    ...
    length: Yup.object().shape({
      years: Yup.number().min(0).max(99, 'El valor máximo permitido es 99').label('años de residencia').required(),
      months: Yup.number()
        .positive()
        .min(0)
  ...
  curp: Yup.string()
    .matches(
      /^([A-Z][AEIOUX][A-Z]{2}\d{2}(?:0[1-9]|1[0-2])(?:0[1-9]|[12]\d|3[01])[HM](?:AS|B[CS]|C[CLMSH]|D[FG]|G[TR]|HG|JC|M[CNS]|N[ETL]|OC|PL|Q[TR]|S[PLR]|T[CSL]|VZ|YN|ZS)[B-DF-HJ-NP-TV-Z]{3}[0-9A-J][0-9])$/i,
      {
